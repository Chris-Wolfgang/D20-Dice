...
#pragma warning restore CA1508 // Avoid dead conditional code
    ...
    [InlineData("d20", 20)]
    [InlineData("d2", 2)]
    public void TryParse_when_notation_is_valid_returns_new_Die(string notation, int sideCount)
    {
        var result = Die.TryParse(notation);
    ...
    [InlineData(null)]
    [InlineData("")]
    public void TryParse_when_notation_is_null_or_whitespace(string? notation)
    {
        var result = Die.TryParse(notation);
    ...
    [Theory]
    [InlineData("6")]      // missing the 'd'
    [InlineData("2d6")]    // a single die has no count greater than 1
    [InlineData("dx")]
    [InlineData("xd6")]
    public void TryParse_when_notation_is_invalid_fails_with_error_message(string notation)
    {
        var result = Die.TryParse(notation);

        Assert.True(result.Failed);
        Assert.Equal("Invalid die notation format. Value must be in dY format.", result.ErrorMessage);
    }
    ...
    [InlineData("d1")]
    [InlineData("d0")]
    public void TryParse_when_side_count_is_less_than_2_fails_with_error_message(string notation)
    {
        var result = Die.TryParse(notation);
